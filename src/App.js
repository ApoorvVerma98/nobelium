import React, { useState } from 'react';/* it imports the React library and the useState hook from it. 
  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <p>Count: {count}</p>
        <button onClick={() => setCount(count + 1)}>Increase</button> {/*function with a new value that is one greater than the current count state variable.*/}
        <button onClick={() => setCount(count - 1)}>Decrease</button> {/*function with a new value that is one less than the current count state variable.*/}
        <p>Name: {name}</p>
